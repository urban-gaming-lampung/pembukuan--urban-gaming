...
exports.rotateMonthlyGame = exports.validateAndSubmitScore = exports.getServerTime = void 0;
exports.getJakartaMonthKey = getJakartaMonthKey;
exports.getJakartaStartOfTodayMs = getJakartaStartOfTodayMs;
 ...
 */
function getJakartaStartOfTodayMs(date = new Date()) {
    const formatter = new Intl.DateTimeFormat('en-US', {
        timeZone: 'Asia/Jakarta',
        year: 'numeric',
        month: '2-digit',
 ...
 */
exports.validateAndSubmitScore = (0, https_1.onCall)(async (request) => {
    const auth = request.auth;
    if (!auth) {
        throw new https_1.HttpsError('unauthenticated', 'Pengguna tidak terautentikasi.');
    }
    const email = auth.token.email?.toLowerCase().trim();
    if (!email) {
        throw new https_1.HttpsError('invalid-argument', 'Email tidak ditemukan.');
    }
    ...
    if (typeof score !== 'number' || !gameType || !monthKey) {
        throw new https_1.HttpsError('invalid-argument', 'Parameter input tidak lengkap atau tidak valid.');
    }
    // 1. Validate score limits
    const cap = MAX_SCORE_CAP[gameType];
    if (cap === undefined) {
        throw new https_1.HttpsError('invalid-argument', `Tipe game "${gameType}" tidak didukung.`);
    }
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', err.message || 'Terjadi kesalahan sistem internal.');
    }
});
/**
 ...
 */
exports.rotateMonthlyGame = (0, scheduler_1.onSchedule)({
    schedule: '0 0 1 * *',
    timeZone: 'Asia/Jakarta',
}, async (event) => {
