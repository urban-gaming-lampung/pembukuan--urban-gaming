...
import { hapticScore, hapticGameOver, hapticHighScore } from '../engine/haptics';
import { PixelConfetti } from '../engine/confetti';
import { ScreenShake } from '../engine/screenShake';
import { snakeReducer, INITIAL_STATE } from './snakeReducer';
import { renderSnakeGame } from './snakeRenderer';
...
/**
 * SnakeGame component composes the complete retro gaming experience.
 * Combines Handheld frame, Canvas GameScreen, DPad, Action buttons, native audio,
 * and Firebase secure score submission.
 */
export default function SnakeGame({ onClose, monthKey, currentHighScore }: SnakeGameProps) {
  const [gameState, dispatch] = useReducer(snakeReducer, null, INITIAL_STATE);
  ...
  const {
    ...
    error: submitError,
    optimisticHighScore,
  } = useGameSession(monthKey, 'snake', currentHighScore);
  ...
  const handleGameInput = (action: GameAction, isPressed: boolean) => {
    ...
    if (isPressed) {
      if (action === 'UP') dispatch({ type: 'CHANGE_DIRECTION', direction: 'UP' });
      else if (action === 'DOWN') dispatch({ type: 'CHANGE_DIRECTION', direction: 'DOWN' });
      else if (action === 'LEFT') dispatch({ type: 'CHANGE_DIRECTION', direction: 'LEFT' });
      else if (action === 'RIGHT') dispatch({ type: 'CHANGE_DIRECTION', direction: 'RIGHT' });
    }
  };

  const { pressed, triggerAction } = useGameInput(handleGameInput);

  // Game tick loop (60fps animation updates, physics update driven by adaptive speed)
  useGameLoop((dt) => {
    ...
    if (gameStatus === 'PLAYING' && !isCheated) {
      ...
      // Physics tick
      if (accumulatorRef.current >= gameState.speed) {
    ...
    // Render loop (always draws, keeping animations responsive)
    // Setup canvas dimension once or keep fixed resolution (400x300 pixel grid)
    if (canvas.width !== 400) {
      canvas.width = 400;
      canvas.height = 300;
    }
    ...
    renderSnakeGame(ctx, gameState, isDark, elapsedTimeRef.current);

    if (shakeOffset.x !== 0 || shakeOffset.y !== 0) {
      ctx.restore();
    }

    // Draw confetti overlay (on top of game)
  ...
  const handleGameOver = async () => {
    ...
    // Safety check for cheat/devtools detection
    checkDevTools();

    // If limits or cheating flags are caught, stop submission
    if (isCheated) {
    ...
    // Build payload
    const payload = buildPayload(gameState.score, 'snake');
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.isNewHighScore) {
          ...
          hapticHighScore();
          // Trigger confetti burst on new high score
          const canvas = canvasRef.current;
          if (canvas) {
            const ctx = canvas.getContext('2d');
            if (ctx) {
