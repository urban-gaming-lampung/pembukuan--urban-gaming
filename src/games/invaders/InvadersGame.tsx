...
import { invadersReducer, INITIAL_STATE, CANVAS_WIDTH, CANVAS_HEIGHT } from './invadersReducer';
import { renderInvadersGame } from './invadersRenderer';
...
export default function InvadersGame({ onClose, monthKey, currentHighScore }: InvadersGameProps) {
  const [gameState, dispatch] = useReducer(invadersReducer, null, () => INITIAL_STATE(1));
  ...
  const {
    ...
    error: submitError,
    optimisticHighScore,
  } = useGameSession(monthKey, 'invaders', currentHighScore);
  ...
  // Laser shoot sound effect with pitch slide, dual-oscillators and panning
  const playLaserSound = () => {
    ...
    try {
      ...
      const mainGain = ctx.createGain();
      mainGain.gain.setValueAtTime(0.16, now); // Boosted from 0.04
      mainGain.gain.exponentialRampToValueAtTime(0.001, now + duration);
  ...
  useGameLoop((dt) => {
    ...
    if (gameStatus === 'PLAYING' && !isCheated) {
      ...
      const moveSpeed = 160; // px/s
      if (pressed.LEFT) {
        dispatch({ type: 'MOVE_PLAYER', dx: -moveSpeed * dt });
      }
      if (pressed.RIGHT) {
        dispatch({ type: 'MOVE_PLAYER', dx: moveSpeed * dt });
      }
      ...
      dispatch({
        ...
        onHitAlien: (type) => {
          vibrate(12);
          const p = type === 'top' ? 380 : type === 'middle' ? 240 : 150;
          playBeep(p, 'triangle', 0.08, 0.05); // alien explosion sound
        },
        onHitPlayer: () => {
          vibrate(120);
          playBeep(90, 'sawtooth', 0.3, 0.08); // explosion sound
          shakeRef.current.trigger(6, 12);
        },
        onHitBunker: () => {
          vibrate(10);
          playBeep(200, 'triangle', 0.05, 0.04); // block hit sound
        },
        onHitUFO: () => {
          vibrate([40, 20, 40]);
          // High pitch win sound
          playBeep(880, 'sine', 0.15, 0.05);
          setTimeout(() => playBeep(1200, 'sine', 0.2, 0.05), 100);
        },
        onLevelClear: () => {
          vibrate([50, 50, 50]);
          // Victory scale chimes
          playBeep(523.25, 'sine', 0.1);
          setTimeout(() => playBeep(659.25, 'sine', 0.1), 80);
          setTimeout(() => playBeep(783.99, 'sine', 0.1), 160);
          setTimeout(() => playBeep(1046.5, 'sine', 0.35), 240);
        },
        onGameOver: () => {
          vibrate(300);
          playBeep(100, 'sawtooth', 0.45, 0.08);
          handleGameOver();
        },
      });
    }
    ...
    renderInvadersGame(ctx, gameState, isDark, elapsedTimeRef.current);
  ...
  const handleCanvasTouch = (e: React.TouchEvent<HTMLCanvasElement>) => {
    ...
    dispatch({ type: 'PLAYER_TOUCH', x: relativeX });
  };
  ...
  const handleGameOver = async () => {
    ...
    const payload = buildPayload(gameState.score, 'invaders');
