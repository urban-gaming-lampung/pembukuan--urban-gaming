...
import { runnerReducer, INITIAL_STATE, CANVAS_WIDTH, CANVAS_HEIGHT } from './runnerReducer';
import { renderRunnerGame } from './runnerRenderer';
...
export default function RunnerGame({ onClose, monthKey, currentHighScore }: RunnerGameProps) {
  const [gameState, dispatch] = useReducer(runnerReducer, null, INITIAL_STATE);
  ...
  const {
    ...
    error: submitError,
    optimisticHighScore,
  } = useGameSession(monthKey, 'runner', currentHighScore);
  ...
  // Handle keyboard / DPad mapping controls
    ...
    if (isPressed) {
      if (action === 'A' || action === 'UP') {
        dispatch({
          type: 'JUMP',
          onJump: () => {
            playBeep(330, 'sine', 0.08, 0.15); // E4 jump sound
            vibrate(10);
          },
        });
      } else if (action === 'B') {
        dispatch({
          type: 'SLIDE',
          onSlide: () => {
            playBeep(220, 'triangle', 0.12, 0.15); // A3 slide sound
          },
        });
      } else if (action === 'DOWN') {
        dispatch({
          type: 'GROUND_POUND',
          onPound: () => {
            playBeep(160, 'sawtooth', 0.08, 0.18); // downward impact sound
            vibrate(15);
          },
        });
      }
  ...
  useGameLoop((dt) => {
    ...
    if (gameStatus === 'PLAYING' && !isCheated) {
      ...
      dispatch({
        ...
        onScoreCoin: () => {
          vibrate(10);
          // High chime coin collection sound
          playBeep(880, 'sine', 0.1, 0.18);
        },
        onGameOver: () => {
          vibrate(250);
          playBeep(110, 'sawtooth', 0.45, 0.28); // crash gameover sound
    ...
    renderRunnerGame(ctx, gameState, isDark, elapsedTimeRef.current);
  ...
  const handleGameOver = async () => {
    ...
    const payload = buildPayload(gameState.score, 'runner');
