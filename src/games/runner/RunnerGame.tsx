    ...
    if (isPressed) {
      if (action === 'A' || action === 'UP') {
        dispatch({
          type: 'JUMP',
          onJump: () => {
            playBeep(330, 'sine', 0.08, 0.04); // E4 jump sound
            vibrate(10);
          },
        });
      } else if (action === 'B') {
        dispatch({
          type: 'SLIDE',
          onSlide: () => {
            playBeep(220, 'triangle', 0.12, 0.04); // A3 slide sound
          },
        });
      } else if (action === 'DOWN') {
        dispatch({
          type: 'GROUND_POUND',
          onPound: () => {
            playBeep(160, 'sawtooth', 0.08, 0.05); // downward impact sound
            vibrate(15);
          },
        });
      }
  ...
  useGameLoop((dt) => {
    ...
    if (gameStatus === 'PLAYING' && !isCheated) {
      ...
      dispatch({
        ...
        onGameOver: () => {
          vibrate(250);
          playBeep(110, 'sawtooth', 0.45, 0.08); // crash gameover sound
