...
import { atomicSaveGaji, atomicAddDenda, atomicEnsureGajiRecords, normalizeEmail } from "../lib/salaryService";
...
export default function TabPegawai({ history = [], isOwner = false }: { history?: any[]; isOwner?: boolean }) {
  ...
  // Auto-persist ongkir stub records to Firestore when detected (menggunakan atomicEnsureGajiRecords)
  ...
  useEffect(() => {
    ...
    pegawaiData.forEach(async (p: any) => {
      ...
      const persistKey = `${cleanEmail}_${autoRecords.map((r: any) => r.bulanTahun).join(',')}`;
      if (autoPersistedRef.current.has(persistKey)) return;
      autoPersistedRef.current.add(persistKey);

      try {
        const basePokok = Number(p.gajiPokok) || 1500000;
        const autoMonths = autoRecords.map((r: any) => ({ bulanTahun: r.bulanTahun, id: r.id }));
        await atomicEnsureGajiRecords(cleanEmail, autoMonths, basePokok);
      } catch (err) {
        console.error("Gagal auto-persist records gaji:", err);
        autoPersistedRef.current.delete(persistKey);
      }
    });
  }, [pegawaiData, isGajiLoaded]);

  // === AUTO PENALTY: Tidak Absen Pulang ===
  // Diproses secara sekuensial dan atomik untuk menjamin tidak ada denda yang saling menimpa
  const autoPenaltyPulangRef = React.useRef<Set<string>>(new Set());
  const isProcessingPenaltyRef = React.useRef(false);

  useEffect(() => {
    if (!isLogAbsensiLoaded || !isGajiLoaded) return;
    if (logAbsensi.length === 0) return;
    if (isProcessingPenaltyRef.current) return;

    const processAutoPenalties = async () => {
      isProcessingPenaltyRef.current = true;
      const now = new Date();
      ...
      logAbsensi.forEach(l => {
        const cleanEmail = normalizeEmail(l.email);
        const key = `${cleanEmail}_${l.tanggal}`;
        if (!absenMap.has(key)) absenMap.set(key, { masuk: false, pulang: false, shift: l.shift || "", tanggal: l.tanggal, email: cleanEmail });
      ...
      for (const entry of Array.from(absenMap.values())) {
        // Only process if: has Masuk, no Pulang
        if (!entry.masuk || entry.pulang) continue;
        ...
        if (now < deadline) continue; // Belum lewat batas waktu

        // Idempotency: skip if already processed in session
        const idempKey = `noCheckOut_${entry.email}_${entry.tanggal}`;
        if (autoPenaltyPulangRef.current.has(idempKey)) continue;
        ...
        try {
          ...
          await atomicAddDenda(entry.email, dendaItem, currentBulanTahun);
          console.log(`[Auto-Penalty] Berhasil memproses tidak absen pulang: ${entry.email} tanggal ${entry.tanggal}`);
        } catch (e) {
          console.error("Auto penalty pulang error:", e);
          autoPenaltyPulangRef.current.delete(idempKey); // Allow retry on failure
        }
      }
      isProcessingPenaltyRef.current = false;
    };

    processAutoPenalties();
  }, [logAbsensi, isLogAbsensiLoaded, isGajiLoaded, absenConfig]);
  ...
  const handleSaveGaji = async (email: string, records: any[]) => {
      try {
        ...
        await atomicSaveGaji(cleanEmail, records);
...
const PegawaiCard = ({ pegawai, onSave, isOwner = false }: any) => {
    ...
    // Sync state when DB updates, tetapi jangan timpa jika Owner sedang aktif mengedit data
    const isDirtyRef = React.useRef(false);
    useEffect(() => {
        isDirtyRef.current = JSON.stringify(records) !== JSON.stringify(pegawai.records || []);
    }, [records, pegawai.records]);
